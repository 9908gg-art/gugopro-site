...
(function () {
    ...
    function parsePageRange(value, total) {
        var pages = {}, parts = String(value || '').split(',');
        parts.forEach(function (part) { var clean = part.trim(); if (!clean) return; var range = clean.split('-').map(function (item) { return Number(item.trim()); }); if (range.length === 1 && Number.isInteger(range[0]) && range[0] >= 1 && range[0] <= total) pages[range[0] - 1] = true; else if (range.length === 2 && Number.isInteger(range[0]) && Number.isInteger(range[1]) && range[0] >= 1 && range[1] <= total && range[0] <= range[1]) for (var n = range[0]; n <= range[1]; n += 1) pages[n - 1] = true; });
        return Object.keys(pages).map(Number).sort(function (a, b) { return a - b; });
    }

    function initPdfRemovePages(kind) {
        var file = null, pageCount = 0, outputBlob = null, outputName = 'edited-document.pdf';
        var list = t(kind, 'list'), run = t(kind, 'run'), download = t(kind, 'download'), clear = t(kind, 'clear'), stat = t(kind, 'status'), output = t(kind, 'output-text');
        function add(incoming) { file = incoming.filter(isPdf)[0] || null; pageCount = 0; outputBlob = null; run.disabled = true; download.disabled = true; if (!file) return status(stat, text('請選擇 PDF 檔案。', 'Choose a PDF file.'), 'error'); renderFiles(list, [file], { icon: 'fa-solid fa-file-pdf', remove: false }); C.readArrayBuffer(file).then(function (buffer) { return PDFLib.PDFDocument.load(buffer); }).then(function (doc) { pageCount = doc.getPageCount(); run.disabled = false; status(stat, text('已讀取 ' + pageCount + ' 頁，輸入頁碼範圍即可刪除或提取。', pageCount + ' pages ready. Enter a page range to remove or extract.'), 'success'); }).catch(function () { status(stat, text('無法讀取此 PDF。', 'This PDF could not be read.'), 'error'); }); }
        function execute() { if (!file || !pageCount) return status(stat, text('請先選擇可讀取的 PDF。', 'Choose a readable PDF first.'), 'error'); if (!requireLib(kind, ['PDFLib'])) return; var selected = parsePageRange(t(kind, 'range').value, pageCount), mode = t(kind, 'mode').value; if (!selected.length) return status(stat, text('請輸入有效頁碼，例如 1, 3-5。', 'Enter a valid page range such as 1, 3-5.'), 'error'); var chosen = {}; selected.forEach(function (index) { chosen[index] = true; }); var indices = []; for (var i = 0; i < pageCount; i += 1) if ((mode === 'extract' && chosen[i]) || (mode === 'remove' && !chosen[i])) indices.push(i); if (!indices.length) return status(stat, text('刪除後不能沒有頁面。', 'Removing these pages would leave an empty PDF.'), 'error'); run.disabled = true; download.disabled = true; progress(kind, 10); C.readArrayBuffer(file).then(function (buffer) { return PDFLib.PDFDocument.load(buffer); }).then(function (source) { return PDFLib.PDFDocument.create().then(function (target) { return target.copyPages(source, indices).then(function (pages) { pages.forEach(function (page) { target.addPage(page); }); progress(kind, 75); return target.save(); }); }); }).then(function (bytes) { outputBlob = new Blob([bytes], { type: 'application/pdf' }); outputName = safeBase(file.name) + (mode === 'extract' ? '-extracted.pdf' : '-pages-removed.pdf'); if (output) output.textContent = text('已處理 ' + selected.length + ' 個頁碼，可以下載。', selected.length + ' selected page(s) processed. Download is ready.'); progress(kind, 100); status(stat, text('PDF 頁面處理完成，可以下載。', 'PDF page processing is complete and ready to download.'), 'success'); download.disabled = false; run.disabled = false; }).catch(function (error) { progress(kind, 0); status(stat, text('PDF 頁面處理失敗：' + (error.message || '格式不受支援。'), 'PDF page processing failed: ' + (error.message || 'the format may not be supported.')), 'error'); run.disabled = false; }); }
        bind(kind, add); run.addEventListener('click', execute); download.addEventListener('click', function () { if (outputBlob) C.downloadBlob(outputBlob, outputName); }); clear.addEventListener('click', function () { file = null; pageCount = 0; outputBlob = null; list.innerHTML = ''; if (output) output.textContent = text('等待輸出檔案。', 'Waiting for an output file.'); run.disabled = true; download.disabled = true; progress(kind, 0); status(stat, text('請選擇一個 PDF 檔案。', 'Choose one PDF file.')); });
    }

    function init(kind) {
        if (kind === 'pdf-remove-pages') return initPdfRemovePages(kind);
