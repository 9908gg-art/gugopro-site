(function (root) {
  ...
  var DEFAULT_TASK_ROOMS = IS_EN ? [
    ...
    { id: 'pdf_task_default_3', name: 'Task 3 · Data Extraction', rule: 'Extract financial data, tables, units, and periods from the document into structured Markdown, citing source pages and flagging suspected contradictions.', summary: 'Turn tables and financial data into Markdown.' },
    ...
    { id: 'pdf_task_default_3', name: '任務三 · 數據提煉', rule: '擷取目前文件中的財務數據、表格、單位與期間，整理成結構化 Markdown，標出來源頁碼並指出疑似矛盾。', summary: '將表格與財務數據轉成 Markdown。' },
  ...
  function makeGeneralRoom() {
    return { id: 'pdf_general_analysis', name: IS_EN ? 'General Document Analysis' : '一般文件分析房間', createdAt: new Date().toISOString(), messages: [], pdf: null, taskRule: '', taskSummary: IS_EN ? 'Ask questions about the current document or define your own rule.' : '直接詢問目前文件，或建立自己的核心任務規則。' };
  }
  ...
  function ensureGeneralTaskRoom() {
    ...
    general.name = IS_EN ? 'General Document Analysis' : '一般文件分析房間';
  ...
  function normalizePdfRecord(pdf) {
    ...
    return {
      fileName: text(pdf.fileName || (IS_EN ? 'Document text layer backup' : '文件文字層備份')).slice(0, 180),
      kind: text(pdf.kind || 'txt'),
      fileSize: Number(pdf.fileSize) || 0,
      pageTexts: pageTexts,
      imageDataUrl: /^data:image\//i.test(text(pdf.imageDataUrl || '')) ? text(pdf.imageDataUrl).slice(0, 5000000) : '',
  ...
  function normalizeRoom(room) {
    ...
    return {
      ...
      name: text(item.name || (IS_EN ? 'Document analysis room' : '文件分析房間')).slice(0, 80),
  ...
  function loadRooms() {
    ...
    var legacyGeneral = rooms.find(function (room) { return (room.name === '一般 PDF 分析' || room.name === '一般 PDF 分析房間' || room.name === '一般文件分析房間' || room.name === 'General PDF Analysis' || room.name === 'General Document Analysis') && !room.taskRule && !room.pdf && !room.messages.length; });
    if (legacyGeneral) { legacyGeneral.id = 'pdf_general_analysis'; legacyGeneral.name = IS_EN ? 'General Document Analysis' : '一般文件分析房間'; }
  ...
  function renderRoomName() {
    ...
    if (ruleSummary) ruleSummary.textContent = room && room.taskRule ? (room.taskSummary || (IS_EN ? 'Saved custom task rule' : '已保存自訂任務規則')) : (IS_EN ? 'Ask questions about the current document or define your own rule.' : '直接詢問目前文件，或建立自己的核心任務規則。');
  }
  ...
  function saveRuntimeForActiveRoom() {
    ...
    if (snapshot && snapshot.file) runtimeByRoom.set(activeRoomId, snapshot);
    var room = getActiveRoom();
    if (room && snapshot && snapshot.file && snapshot.pageTexts && Object.keys(snapshot.pageTexts).length) {
      room.pdf = { fileName: snapshot.file.name, fileSize: snapshot.file.size, kind: snapshot.documentKind || (snapshot.pdf ? 'pdf' : 'txt'), mimeType: snapshot.file.type || '', imageDataUrl: snapshot.documentImageDataUrl && snapshot.documentImageDataUrl.length <= 5000000 ? snapshot.documentImageDataUrl : '', pageTexts: snapshot.pageTexts || {}, pageOrder: snapshot.pageOrder, currentPage: snapshot.currentPage, savedAt: new Date().toISOString() };
      saveRooms();
    }
  }
  ...
  function roomMessageMeta(room) {
    return (room.pdf ? (IS_EN ? 'Document text layer · ' : '文件文字層 · ') : '') + room.messages.length + (IS_EN ? ' messages' : ' 則對話') + (room.taskRule ? (IS_EN ? ' · task saved' : ' · 已設定任務') : '');
  }
  ...
  function clearPdfForRoom() {
    ...
    var pageTexts = room.pdf.pageTexts || {};
    var storedKind = room.pdf.kind || 'txt';
    var isPdfTextBackup = storedKind === 'pdf';
    var kind = isPdfTextBackup ? 'txt' : storedKind;
    var mimeType = isPdfTextBackup ? 'text/plain' : (room.pdf.mimeType || (kind === 'image' ? 'image/png' : 'text/plain'));
    var pseudoFile = { name: room.pdf.fileName || (IS_EN ? 'Document text backup.txt' : '文件文字層備份.txt'), size: room.pdf.fileSize || 0, type: mimeType };
    var snapshot = { file: pseudoFile, pdf: null, documentKind: kind, documentText: Object.keys(pageTexts).sort(function (a, b) { return Number(a) - Number(b); }).map(function (page) { return pageTexts[page]; }).join('\n\n'), documentImageDataUrl: room.pdf.imageDataUrl || '', pageTexts: pageTexts, pageOrder: room.pdf.pageOrder || Object.keys(pageTexts).map(Number), currentPage: room.pdf.currentPage || 1, pageRotations: {}, textReady: true, zoom: .92, fitMode: 'fit-width', tool: 'select', annotations: {}, annotationImages: {}, signatures: {}, activeSignatureId: null, outline: [] };
    if (root.GugoProPdfSuite && root.GugoProPdfSuite.restoreSnapshot) root.GugoProPdfSuite.restoreSnapshot(snapshot);
    if ($('pdf-reader-status')) $('pdf-reader-status').innerHTML = '<i class="fa-solid fa-file-lines"></i> ' + (isPdfTextBackup ? (IS_EN ? 'PDF text backup ready' : 'PDF 文字層備份就緒') : (IS_EN ? 'Document text ready' : '文件文字層就緒'));
    if ($('pdf-status')) $('pdf-status').textContent = isPdfTextBackup ? (IS_EN ? 'A local PDF text backup is loaded; reopen the original PDF to edit pages.' : '房間已載入本機 PDF 文字層備份；如需編輯頁面，請重新開啟原始 PDF。') : (IS_EN ? 'A local document text backup is loaded; AI can continue analysis.' : '房間已載入本機文件文字層備份，AI 可繼續分析。');
  }
  ...
  async function switchRoom(roomId, force) {
    ...
    try {
      ...
      showRoomNotice((IS_EN ? 'Switched to “' : '已切換至「') + room.name + (IS_EN ? '”; chat and document text stay isolated.' : '」；對話與文件文字層彼此獨立。'), 'success');
  ...
  function deleteRooms(ids) {
    ...
    if (rooms.length <= selected.length) { showRoomNotice(IS_EN ? 'Keep at least one document analysis room.' : '至少需保留一個文件分析房間。', 'error'); return; }
    if (!root.confirm(IS_EN ? 'Delete the selected rooms? Their chats and saved document text layers will also be removed.' : '確定刪除選取房間嗎？對話與已保存的文件文字層會一併移除。')) return;
  ...
  function showRoomOps(operation) {
    ...
    $('pdf-room-ops-title').textContent = operation === 'export' ? (IS_EN ? 'Export document analysis rooms' : '匯出文件分析房間') : (IS_EN ? 'Clear or delete rooms' : '清除或刪除房間');
    $('pdf-room-ops-desc').textContent = operation === 'export' ? (IS_EN ? 'Select rooms to export; each includes chat and any saved document text layer.' : '勾選要匯出的房間；每個房間包含對話與可用的文件文字層。') : (IS_EN ? 'Select rooms, then choose whether to clear chats or delete the rooms.' : '勾選房間後，確認時可選擇清空對話或直接刪除房間。');
    list.replaceChildren();
    rooms.forEach(function (room) {
  ...
  function importRooms(file) {
    ...
    reader.onload = function () {
      try {
        ...
        activeRoomId = normalized[0].id; saveRooms(); renderRoomList(); switchRoom(activeRoomId, true); showRoomNotice((IS_EN ? 'Imported ' : '已匯入 ') + normalized.length + (IS_EN ? ' document analysis room(s).' : ' 個文件分析房間。'), 'success');
  ...
  function handleTextExtracted(snapshot) {
    ...
    room.pdf = { fileName: snapshot.file.name, fileSize: snapshot.file.size, kind: snapshot.documentKind || (snapshot.pdf ? 'pdf' : 'txt'), mimeType: snapshot.file.type || '', imageDataUrl: snapshot.documentImageDataUrl && snapshot.documentImageDataUrl.length <= 5000000 ? snapshot.documentImageDataUrl : '', pageTexts: snapshot.pageTexts || {}, pageOrder: snapshot.pageOrder || [], currentPage: snapshot.currentPage || 1, savedAt: new Date().toISOString() };
  ...
  function bind() {
    ...
    root.addEventListener('gugopro:document-loaded', handlePdfLoaded);
    root.addEventListener('gugopro:pdf-text-extracted', function (event) { handleTextExtracted(event.detail || {}); });
    root.addEventListener('gugopro:document-text-extracted', function (event) { handleTextExtracted(event.detail || {}); });
