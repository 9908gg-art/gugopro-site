/* GugoPro local video tools: browser-only Canvas, MediaRecorder, Web Audio, JSZip and gifshot. */
(function () {
  'use strict';
  ...
  var T = en ? {
    ...
    unsupported: 'This browser does not support the required local media API.', rendering: 'Rendering locally…'
  } : {
    ...
    unsupported: '此瀏覽器不支援必要的本機媒體 API。', rendering: '正在本機渲染…'
  };
  ...
  async function renderVideo(options) {
    ...
    var rotation = Number(options.rotation || 0), flipH = !!options.flipH, flipV = !!options.flipV, includeAudio = options.includeAudio !== false;
    setupCanvas(rotation); video.pause(); video.currentTime = 0; video.playbackRate = Number(options.speed || 1); progress(3); setStatus(T.rendering);
    ...
    function loop() { drawTransformed(rotation, flipH, flipV); progress(Math.min(96, Math.round((video.currentTime / video.duration) * 92) + 4)); if (!ended) raf = requestAnimationFrame(loop); }
    function stop() { if (ended) return; ended = true; cancelAnimationFrame(raf); drawTransformed(rotation, flipH, flipV); if (recorder.state !== 'inactive') recorder.stop(); }
  ...
  var renderButton = document.getElementById('video-render'); if (renderButton) renderButton.addEventListener('click', function () { var rotation = (document.getElementById('video-rotation') || {}).value || 0, speed = (document.getElementById('video-speed') || {}).value || 1; renderVideo({rotation: rotation, speed: speed, flipH: document.getElementById('video-flip-h') ? document.getElementById('video-flip-h').checked : false, flipV: document.getElementById('video-flip-v') ? document.getElementById('video-flip-v').checked : false, includeAudio: tool !== 'video-mute', suffix: tool === 'video-mute' ? '-muted' : tool === 'video-speed' ? '-speed' : '-rotated'}).catch(onError); });
