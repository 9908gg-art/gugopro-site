...
Final cache-busted verification used `https://gugopro.com/academy/research/quant-lab.html?qa=f40dded`. The page loaded the final commit assets, and pressing the default run button completed the same result: snapshot `sha256:60a9cfa243bcf1a4ded04b59a43aac6cc0b9e74a4cd1c0e65473e78d1e532369`, as-of `2025-08-26 → 2026-08-25`, 261 source rows, 251 complete rows, 10 missing rows excluded without imputation, 231 trainable labels, four held-out windows, and `-1.01%` mean excess return. The page explicitly labeled the result `baseline_only`; it did not promote an ML model or produce a buy/sell status.
